        ...
        const AchievementManager = (function(){
            ...
            */
            ...
            function evaluateAll(metrics, {mode, isReplay, collapseGroups=true}){
                ...
                for (const a of defs) {
                    try {
                        ...
                        if (evalRule(a.condition, metrics)) matched.push(a);
    ...
    <use href="#achv-apple"></use>
        ...
        ;(function defineAchievements(){
            ...
            D({ id:'any_acc_t5', mode:'any', name:'聖潔器皿', desc:'完美限定 命中率≥60%（生存≥2關）', displayTier:5, groupKey:'ladder_any_acc', rung:2, realtime:false,
            ...
            D({ id:'any_acc_t4', mode:'any', name:'聖潔器皿', desc:'完美限定 命中率≥70%（生存≥4關）', displayTier:4, groupKey:'ladder_any_acc', rung:3, realtime:false,
                condition:{ type:'custom', fn:m=> {
            ...
            D({ id:'any_acc_t3', mode:'any', name:'聖潔器皿', desc:'完美限定 命中率≥80%（生存≥6關）', displayTier:3, groupKey:'ladder_any_acc', rung:4, realtime:false,
                condition:{ type:'custom', fn:m=> {
            ...
            D({ id:'any_acc_t2', mode:'any', name:'聖潔器皿', desc:'完美限定 命中率≥90%（生存≥8關）', displayTier:2, groupKey:'ladder_any_acc', rung:5, realtime:false,
                condition:{ type:'custom', fn:m=> {
            ...
            // T3: 完美題數≥15 且 平均≤6s
            // T2: 完美題數≥20 且 平均≤5s
            D({ id:'any_speed_t5', mode:'any', name:'奔跑不困倦', desc:'完美答題≥5題 且 平均作答≤7秒', displayTier:5, groupKey:'ladder_any_speed', rung:2, realtime:false,
                condition:{ type:'custom', fn:m=> (m.firstTryCorrectCount||0)>=5 && (m.firstTryAvgAnswerMs||0)>0 && m.firstTryAvgAnswerMs<=7000 } });
            D({ id:'any_speed_t4', mode:'any', name:'奔跑不困倦', desc:'完美答題≥10題 且 平均作答≤6.5秒', displayTier:4, groupKey:'ladder_any_speed', rung:3, realtime:false,
                condition:{ type:'custom', fn:m=> (m.firstTryCorrectCount||0)>=10 && (m.firstTryAvgAnswerMs||0)>0 && m.firstTryAvgAnswerMs<=6500 } });
            D({ id:'any_speed_t3', mode:'any', name:'奔跑不困倦', desc:'完美答題≥15題 且 平均作答≤5.5秒', displayTier:3, groupKey:'ladder_any_speed', rung:4, realtime:false,
                condition:{ type:'custom', fn:m=> (m.firstTryCorrectCount||0)>=15 && (m.firstTryAvgAnswerMs||0)>0 && m.firstTryAvgAnswerMs<=5500 } });
            D({ id:'any_speed_t2', mode:'any', name:'奔跑不困倦', desc:'完美答題≥20題 且 平均作答≤5秒', displayTier:2, groupKey:'ladder_any_speed', rung:5, realtime:false,
            ...
            D({ id:'any_streak_t5', mode:'any', name:'葡萄枝子', desc:'連擊≥5', displayTier:5, groupKey:'ladder_any_streak', rung:2, condition:{ type:'metric', field:'maxComboReached', op:'>=', value:5 } });
            D({ id:'any_streak_t4', mode:'any', name:'葡萄枝子', desc:'連擊≥10', displayTier:4, groupKey:'ladder_any_streak', rung:3, condition:{ type:'metric', field:'maxComboReached', op:'>=', value:10 } });
            D({ id:'any_streak_t3', mode:'any', name:'葡萄枝子', desc:'連擊≥20', displayTier:3, groupKey:'ladder_any_streak', rung:4, condition:{ type:'metric', field:'maxComboReached', op:'>=', value:20 } });
            D({ id:'any_streak_t2', mode:'any', name:'葡萄枝子', desc:'連擊≥35', displayTier:2, groupKey:'ladder_any_streak', rung:5, condition:{ type:'metric', field:'maxComboReached', op:'>=', value:35 } });

            // Any — 低失誤平均（重建城牆）：與聖潔器皿一致採用「生存完成關數門檻」
            // 門檻：生存至少達 2/4/6/8 關（完成關，以 5 題一關計算），同時每關平均失誤 ≤ 4/3/2/1（闖關時沿用 classicAvg）
            D({ id:'any_low_miss_t5', mode:'any', name:'重建城牆', desc:'每關平均失誤≤4（生存≥2關）', displayTier:5, groupKey:'ladder_any_lowmiss', rung:2, realtime:false,
                condition:{ type:'custom', fn:m=> {
            ...
            D({ id:'any_low_miss_t4', mode:'any', name:'重建城牆', desc:'每關平均失誤≤3（生存≥4關）', displayTier:4, groupKey:'ladder_any_lowmiss', rung:3, realtime:false,
                condition:{ type:'custom', fn:m=> {
            ...
            D({ id:'any_low_miss_t3', mode:'any', name:'重建城牆', desc:'每關平均失誤≤2（生存≥6關）', displayTier:3, groupKey:'ladder_any_lowmiss', rung:4, realtime:false,
                condition:{ type:'custom', fn:m=> {
            ...
            D({ id:'any_low_miss_t2', mode:'any', name:'重建城牆', desc:'每關平均失誤≤1（生存≥8關）', displayTier:2, groupKey:'ladder_any_lowmiss', rung:5, realtime:false,
                condition:{ type:'custom', fn:m=> {
            ...
            D({ id:'c_levels_perfect_t5', mode:'classic', name:'無瑕無疵', desc:'本局完美關卡≥1', displayTier:5, groupKey:'ladder_c_levels', rung:2, condition:{ type:'custom', fn:m=> (m.levelsPerfectCount||0)>=1 } });
            D({ id:'c_levels_perfect_t4', mode:'classic', name:'無瑕無疵', desc:'本局完美關卡≥3', displayTier:4, groupKey:'ladder_c_levels', rung:3, condition:{ type:'custom', fn:m=> (m.levelsPerfectCount||0)>=3 } });
            D({ id:'c_levels_perfect_t3', mode:'classic', name:'無瑕無疵', desc:'本局完美關卡≥5', displayTier:3, groupKey:'ladder_c_levels', rung:4, condition:{ type:'custom', fn:m=> (m.levelsPerfectCount||0)>=5 } });
            D({ id:'c_levels_perfect_t2', mode:'classic', name:'無瑕無疵', desc:'本局完美關卡≥7', displayTier:2, groupKey:'ladder_c_levels', rung:5, condition:{ type:'custom', fn:m=> (m.levelsPerfectCount||0)>=7 } });

            // Survival — 恆久忍耐（以時長為主，門檻：T5≥90s, T4≥120s, T3≥150s, T2≥180s）
            D({ id:'s_duration_t5', mode:'survival', name:'恆久忍耐', desc:'存活≥90秒', displayTier:5, groupKey:'ladder_s_duration', rung:2, condition:{ type:'custom', fn:m=> (m.survivalDuration||0)>=90 } });
            D({ id:'s_duration_t4', mode:'survival', name:'恆久忍耐', desc:'存活≥120秒', displayTier:4, groupKey:'ladder_s_duration', rung:3, condition:{ type:'custom', fn:m=> (m.survivalDuration||0)>=120 } });
            D({ id:'s_duration_t3', mode:'survival', name:'恆久忍耐', desc:'存活≥150秒', displayTier:3, groupKey:'ladder_s_duration', rung:4, condition:{ type:'custom', fn:m=> (m.survivalDuration||0)>=150 } });
            D({ id:'s_duration_t2', mode:'survival', name:'恆久忍耐', desc:'存活≥180秒', displayTier:2, groupKey:'ladder_s_duration', rung:5, condition:{ type:'custom', fn:m=> (m.survivalDuration||0)>=180 } });

            // Survival — 警醒守望：計時在 60–90 秒區間的累計秒數（T5/T4/T3/T2: 50/75/100/125）
            D({ id:'s_time_band_60_90_t5', mode:'survival', name:'警醒守望', desc:'60–90秒區間累計≥50秒', displayTier:5, groupKey:'ladder_s_band_60_90', rung:2, realtime:false,
                condition:{ type:'metric', field:'timeInBand60to90', op:'>=', value:50 } });
            D({ id:'s_time_band_60_90_t4', mode:'survival', name:'警醒守望', desc:'60–90秒區間累計≥75秒', displayTier:4, groupKey:'ladder_s_band_60_90', rung:3, realtime:false,
                condition:{ type:'metric', field:'timeInBand60to90', op:'>=', value:75 } });
            D({ id:'s_time_band_60_90_t3', mode:'survival', name:'警醒守望', desc:'60–90秒區間累計≥100秒', displayTier:3, groupKey:'ladder_s_band_60_90', rung:4, realtime:false,
                condition:{ type:'metric', field:'timeInBand60to90', op:'>=', value:100 } });
            D({ id:'s_time_band_60_90_t2', mode:'survival', name:'警醒守望', desc:'60–90秒區間累計≥125秒', displayTier:2, groupKey:'ladder_s_band_60_90', rung:5, realtime:false,
            ...
            D({ id:'t1_morning_star', mode:'classic', name:'明亮晨星', desc:'開局5關皆完美答題且均速≤5秒', displayTier:1, realtime:false,
                condition:{ type:'custom', fn:m=> {
                    ...
                    return true; } } });
            D({ id:'t1_saints_endurance', mode:'survival', name:'聖徒忍耐', desc:'存活≥240秒（未使用提示）', displayTier:1, realtime:false,
            ...
            D({ id:'t1_blameless_worship', mode:'any', name:'辛苦勞碌', desc:'平均作答≤3秒（生存≥8關）', displayTier:1, realtime:false,
                condition:{ type:'custom', fn:m=> {
            ...
            D({ id:'t1_kings_way', mode:'classic', name:'成聖之路', desc:'連續3關完美答題且各關開局3題均速≤3秒', displayTier:1, realtime:false,
                condition:{ type:'custom', fn:m=> {
            ...
            // 生存：風平浪靜（時間曲線穩定）
            D({ id:'t1_calm_sea', mode:'survival', name:'風平浪靜', desc:'存活≥210秒且時間波動低（≤6）', displayTier:1, realtime:false,
                condition:{ type:'custom', fn:m=> (m.survivalDuration||0)>=210 && (m.timeStdDev||999) <= 6 } });
            // 生存：復活的大能（瀕死多次回升）
            D({ id:'t1_resurrection_power', mode:'survival', name:'死裡復活', desc:'存活≥180秒且瀕死回升≥3次', displayTier:1, realtime:false,
                condition:{ type:'custom', fn:m=> (m.survivalDuration||0)>=180 && (m.nearDeathRecoveries||0) >= 3 } });

            // 山寨高台（任一模式）：每關錯誤上限 + 完美關卡下限
            D({ id:'any_stability_t5', mode:'any', name:'山寨高台', desc:'每關錯誤≤3 且 完美關卡≥1', displayTier:5, groupKey:'ladder_any_stability', rung:2, realtime:false,
            ...
            D({ id:'any_stability_t4', mode:'any', name:'山寨高台', desc:'每關錯誤≤2 且 完美關卡≥2', displayTier:4, groupKey:'ladder_any_stability', rung:3, realtime:false,
            ...
            D({ id:'any_stability_t3', mode:'any', name:'山寨高台', desc:'每關錯誤≤1 且 完美關卡≥3', displayTier:3, groupKey:'ladder_any_stability', rung:4, realtime:false,
            ...
            D({ id:'any_stability_t2', mode:'any', name:'山寨高台', desc:'每關錯誤=0 且 完美關卡≥4', displayTier:2, groupKey:'ladder_any_stability', rung:5, realtime:false,
            ...
            D({ id:'s_time_band_0_30_t5', mode:'survival', name:'耐心等候', desc:'0–30秒區間累計≥45秒', displayTier:5, groupKey:'ladder_s_band_0_30', rung:2, realtime:false,
                condition:{ type:'metric', field:'timeInBand0to30', op:'>=', value:45 } });
            D({ id:'s_time_band_0_30_t4', mode:'survival', name:'耐心等候', desc:'0–30秒區間累計≥60秒', displayTier:4, groupKey:'ladder_s_band_0_30', rung:3, realtime:false,
                condition:{ type:'metric', field:'timeInBand0to30', op:'>=', value:60 } });
            D({ id:'s_time_band_0_30_t3', mode:'survival', name:'耐心等候', desc:'0–30秒區間累計≥75秒', displayTier:3, groupKey:'ladder_s_band_0_30', rung:4, realtime:false,
                condition:{ type:'metric', field:'timeInBand0to30', op:'>=', value:75 } });
            D({ id:'s_time_band_0_30_t2', mode:'survival', name:'耐心等候', desc:'0–30秒區間累計≥90秒', displayTier:2, groupKey:'ladder_s_band_0_30', rung:5, realtime:false,
                condition:{ type:'metric', field:'timeInBand0to30', op:'>=', value:90 } });

            // T1 Specials（可擇一定義，避免與梯隊重疊）
            // 聖靈充滿：改為 Any 類別；文案更新（條件不變）
            D({ id:'t1_chariot_fire', mode:'any', name:'聖靈充滿', desc:'未使用提示 且每關平均失誤≤1', displayTier:1, realtime:false,
                condition:{ type:'custom', fn:m=> (m.hintsUsed||0) === 0 && (m.avgMistakesPerLevel||0) <= 1 } });
            // 分別為聖：完美限定 命中率≥99% 且 ≥25題
            // 分別為聖（調整後）：完美命中≥97%（≥24題）且完美平均作答≤5秒
            // 目的：與「真理與慈愛」（全程完美答題）區隔，增加速度要求
            D({ id:'t1_near_holy', mode:'any', name:'分別為聖', desc:'完美限定 命中≥97% 平均≤5秒', displayTier:1, realtime:false,
                condition:{ type:'custom', fn:m=>
