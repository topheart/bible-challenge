            ...
            try {
                if (!gameState._replaySequence && !isEquipMode) {
                    ...
                    if (gameRecord && Array.isArray(gameRecord.achievements) && gameRecord.achievements.length > 0) {
                        ...
                        unlocked = AchievementManager.evaluateAll(m || {}, {mode, isReplay:false});
    ...
    function spawnConfettiRain(delta, originRect) {
            try {
                ...
                let base = 8;
                if (abs >= 300) base = 26;
                else if (abs >= 150) base = 22;
                else if (abs >= 60) base = 16;
                else if (abs >= 20) base = 10;
                let count = Math.min(50, base);
                if (getReducedMotion()) count = Math.min(count, 10);
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    const h = Math.max(10, w + 4 + Math.floor(Math.random()*10));
                    el.style.width = w + 'px';
                    el.style.height = h + 'px';
                    const spreadFactor = (abs <= 100) ? 0.9 : 0.75; // 加大水平範圍
                    const spread = Math.round(window.innerWidth * spreadFactor);
                    const leftBase = spawnCenter - Math.round(spread / 2);
                    const left = leftBase + Math.floor(Math.random() * spread) + Math.floor((Math.random() - 0.5) * 220);
                    el.style.left = (Math.max(4, Math.min(window.innerWidth - 4, left))) + 'px';
                    el.style.top = startY + 'px';
                    const drift = Math.round((Math.random()*320) - 160) + 'px';
                    el.style.setProperty('--conf-x', drift);
                    el.style.opacity = '1';
                    el.style.transform = `translateY(0)`;
                    el.style.borderRadius = (Math.random() > 0.7 ? '2px' : '6px');
                    document.body.appendChild(el);
                    const life = getReducedMotion() ? 2400 + Math.floor(Math.random()*400) : 5200 + Math.floor(Math.random()*1400);
